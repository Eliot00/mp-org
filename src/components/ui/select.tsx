...
export const SelectTrigger = <T extends ValidComponent = "button">(
  ...
  return (
    <SelectPrimitive.Trigger
      class={cn(
        "border-input ring-offset-background placeholder:text-muted-foreground focus-visible:(ring-1.5 ring-ring) disabled:(cursor-not-allowed opacity-50) flex h-9 w-full items-center justify-between rounded-md border bg-transparent px-3 py-2 text-sm shadow-sm transition-shadow focus:outline-none",
        local.class,
      )}
      {...rest}
    >
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        class={cn(
          "bg-popover text-popover-foreground data-[expanded]:(animate-in fade-in-0 zoom-in-95) data-[closed]:(animate-out fade-out-0 zoom-out-95) relative z-50 min-w-[8rem] overflow-hidden rounded-md border shadow-md",
          local.class,
        )}
        {...rest}
      >
...
export const SelectItem = <T extends ValidComponent = "li">(
  ...
  return (
    <SelectPrimitive.Item
      class={cn(
        "focus:(bg-accent text-accent-foreground) data-[disabled]:(pointer-events-none opacity-50) relative flex w-full cursor-default select-none items-center rounded-sm py-1.5 pl-2 pr-8 text-sm outline-none",
        local.class,
      )}
      {...rest}
    >
